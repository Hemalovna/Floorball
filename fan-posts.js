document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadPosts() {
    return JSON.parse(localStorage.getItem("fanPosts")) || [];
  }

  function savePosts(posts) {
    localStorage.setItem("fanPosts", JSON.stringify(posts));
    // Dispatch a storage event to notify same-tab listeners (some browsers don't fire storage on same tab)
    try {
      window.dispatchEvent(new StorageEvent('storage', { key: 'fanPosts', newValue: JSON.stringify(posts) }));
    } catch (err) {
      // ignore in environments that don't allow constructing StorageEvent
    }
  }
  ...
  // Listen to storage events so posts appear live in other tabs/windows
  window.addEventListener("storage", (e) => {
    if (e.key === "fanPosts") {
      renderPosts();
    }
    if (e.key === "loggedInUser") {
      // If loggedInUser changes in another tab, reload to enforce auth
